---
name: kle-workflow-guide
description: katana-language-editor の開発で、OpenSpec、品質ゲート、自己レビュー、コミット、PR 作成を迷わずつなぐための案内スキル。大きい変更、バグ修正、品質確認、次に使うスキルの判断で必ず使う。
---

# kle Workflow Guide

このスキルは、katana-language-editor（kle）の repo-local skill を組み合わせる入口です。
kle は言語エディタ基盤（language editor runtime）なので、画面 UI、翻訳、アイコン、リリース文書の都合を前提にしません。
...
kle の品質ゲートは、言語エディタの中立 interface と egui 実装の境界を守るために使います。
