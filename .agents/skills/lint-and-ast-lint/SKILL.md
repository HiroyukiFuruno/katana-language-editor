...
description: katana-language-editor の静的検査と抽象構文木検査を厳格に実行する。language editor runtime の品質ゲート、clippy、format、test、禁止パターン確認、allow や exclude の不正追加防止で使う。
---

# Lint and AST Lint

このスキルは、kle の品質ゲートを「通ればよい」ではなく「設計意図を守る検査」として扱います。
言語エディタ基盤（language editor runtime）では、中立 interface に egui 型や host 固有状態を漏らさないことを重視します。
...
次は language editor runtime の品質を落とします。
