...
description: katana-language-editor の OpenSpec tasks.md を、実装者がそのまま進められる粒度へ整える。タスク分割、完了条件、品質ゲート、レビュー停止点を入れるときに使う。
---

# OpenSpec Tasks Template

`tasks.md` は実装メモではなく、別の作業者が読んでも同じ順序で実装できる契約です。
kle では、公開 API、中立 interface、language editor、egui implementation、テスト、品質ゲートを分けて書きます。
...
画面確認ではなく、egui implementation 出力、生成 artifact、reference image、比較結果など、kle の成果物で確認できる形にします。
