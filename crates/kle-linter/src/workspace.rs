...
impl WorkspaceModel {
    pub fn load(root: &Path) -> Result<Self, KleLintError> {
    ...
    fn target_roots(root: &Path) -> Vec<PathBuf> {
        vec![
            root.join("crates/kle-linter/src"),
            ...
            root.join("tools/kle-storybook/src"),
        ]
    }

    fn collect_rs_files(root: &Path) -> Result<Vec<SourceFile>, KleLintError> {
    ...
    fn parse_file(path: &Path) -> Result<SourceFile, KleLintError> {
